...
class GeneralLedgerReport(models.AbstractModel):
    ...
    def _get_move_line_data(self, move_line):
        move_type = move_line.get("move_type", "")
        inscription = ""
        account = self.env['account.account'].browse(move_line['account_id'][0])
        accounttype = account.account_type
        if move_type == "entry":
            if accounttype == "liability_payable":
                inscription = "Платеж поставщику"
            elif accounttype == "asset_receivable":
                inscription = "Оплата покупателя"
            # elif accounttype == "asset_current":
            #     inscription = "Оплата покупателя"
            # else:
            #     inscription = ""
        elif move_type == "out_invoice":
            inscription = "Продажа товаров и услуг"
        # elif move_type == "out_refund":
        #     inscription = "Сторно клиента "
        elif move_type == "in_invoice":
            inscription = "Покупка товаров и услуг "
        # elif move_type == "in_refund":
        #     inscription = "Кредитное обязательство поставщика "
        # elif move_type == "out_receipt":
        #     inscription = "Квитанция продаж"
        # elif move_type == "in_receipt":
        #     inscription = "Квитанция покупки"

        transformed_move_name = f"{inscription} No. {move_line['move_name']}"
        move_line_data = {
            "id": move_line["id"],
            "date": move_line["date"],
            "entry": transformed_move_name,
    ...
    def _get_period_ml_data(
        ...
        # ФИЛЬТРАЦИЯ ДЛЯ АКТА СВЕРКИ: оставляем только значимые проводки
        acc_prt_account_ids = self._get_acc_prt_accounts_ids(company_id, grouped_by)
        
        filtered_move_lines = []
        for ml in move_lines:
            # Пропускаем проводки где и дебет и кредит равны 0
            if ml['debit'] == 0.0 and ml['credit'] == 0.0:
                continue
            
            # Для акта сверки: оставляем только счета расчетов с партнерами
            account_id = ml['account_id'][0]
            if account_id not in acc_prt_account_ids:
                continue
                
            filtered_move_lines.append(ml)
        
        move_lines = filtered_move_lines
        ...
        grouped_moves_count = 0
        
        # Для акта сверки: объединяем проводки по документам для исключения дублирования
        consolidated_moves = {}
        for move_line in move_lines:
            move_id = move_line['move_id'][0]
            partner_id = move_line['partner_id'][0] if move_line['partner_id'] else 0
            account_id = move_line['account_id'][0]
            
            # Создаем уникальный ключ для консолидации
            key = (move_id, partner_id, account_id)
            
            if key not in consolidated_moves:
                consolidated_moves[key] = move_line.copy()
            else:
                # Объединяем суммы дебета и кредита
                existing = consolidated_moves[key]
                existing['debit'] += move_line['debit']
                existing['credit'] += move_line['credit']
                existing['balance'] += move_line['balance']
                existing['amount_currency'] += move_line['amount_currency']
        
        # Преобразуем обратно в список
        move_lines = list(consolidated_moves.values())
        
        # Удаляем проводки с нулевым балансом после консолидации
        final_move_lines = []
        for ml in move_lines:
            if abs(ml['debit']) > 0.01 or abs(ml['credit']) > 0.01:
                final_move_lines.append(ml)
        
        move_lines = final_move_lines
        
        # Получаем список партнерских счетов для группировки
        acc_prt_account_ids = self._get_acc_prt_accounts_ids(company_id, grouped_by)
    ...
    def _create_general_ledger(
        ...
        for acc_id in gen_led_data.keys():
            ...
            has_data = False
            if 'fin_bal' in gen_led_data[acc_id]:
                fin_bal = gen_led_data[acc_id]['fin_bal']
                if any(abs(fin_bal.get(key, 0)) > 0.01 for key in ['debit', 'credit', 'balance']):
                    has_data = True
            
            # Check for move lines
            move_lines_count = 0
            for key, value in gen_led_data[acc_id].items():
                if isinstance(key, int) and key != acc_id:
                    if isinstance(value, dict):
                        # Check if there are move lines in this partner/group
    ...
    def _get_report_values(self, docids, data):
        ...
        return {
            "docids": docids,
            "doc_model": "general.ledger.act_revise.wizard",
            "docs": self.env["general.ledger.act_revise.wizard"].browse(docids),
            "foreign_currency": data["foreign_currency"],
