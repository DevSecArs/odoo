...
class MailTemplate(models.Model):
    ...
    offer_pdf_manual_enabled = fields.Boolean(string='Manually complete PDF documents')
    offer_pdf_company_id = fields.Many2one('res.company', string='PDF documents company', default=lambda self: self.env.company)
    offer_pdf_document_ids = fields.One2many('mail.template.offer.pdf.document', 'template_id', string='PDF documents')
    ...
    def _check_offer_pdf_model(self):
        for template in self.filtered('offer_pdf_manual_enabled'):
            if template.model != 'hr.applicant':
                raise ValidationError(_('Manual PDF documents can only be configured on hr.applicant email templates.'))
