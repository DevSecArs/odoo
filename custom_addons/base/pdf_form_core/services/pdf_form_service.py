"""Narrow, defensive reader and writer for text PDF AcroForms."""
...
def _resolve(value):
    """Resolve an indirect object on every PDF backend supported by Odoo."""
...
def inspect_pdf(document, max_size=None):
    """Validate an AcroForm and describe its unambiguous text fields.

    Only one widget per canonical field is accepted. ``max_size`` is expressed
    in bytes; passing ``None`` applies the conservative shared default.
    """
    max_size = DEFAULT_MAX_PDF_SIZE if max_size is None else max_size
    if not document or max_size is not None and len(document) > max_size:
    ...
    def visit(field, ancestors):
        field = _resolve(field)
        key = _object_key(field)
        if key in ancestors:
            raise ValidationError(_('The PDF contains a cyclic form field hierarchy.'))
        if key in visited:
            raise ValidationError(_('The PDF contains an ambiguous form field hierarchy.'))
        visited.add(key)
    ...
    if missing_widgets:
        raise ValidationError(_(
            'The PDF field "%(name)s" has no widget annotation.',
            name=sorted(missing_widgets)[0],
        ))
...
def render_pdf(document, values, readonly=False, max_size=None):
    """Fill a verified PDF and validate the generated bytes before returning."""
    expected = {str(name): str(value or '') for name, value in values.items()}
    fields = {field['name'] for field in inspect_pdf(document, max_size=max_size)}
    ...
    try:
        ...
        raise ValidationError(_(
            'The PDF could not be filled. Check its form field appearance settings.'
        )) from error
    ...
    try:
        ...
        for widget, definition, name in _walk_widgets(result_reader):
            ...
            if value != expected[name]:
                raise ValidationError(_(
                    'The generated PDF did not preserve the value of field "%(name)s".',
                    name=name,
                ))
            if readonly and not (int(definition.get('/Ff', 0)) & READ_ONLY):
                raise ValidationError(_(
                    'The generated PDF field "%(name)s" is not read-only.',
                    name=name,
                ))
            appearance = _resolve(widget.get('/AP')) if widget.get('/AP') else None
            if not appearance or not appearance.get('/N'):
                raise ValidationError(_(
                    'The generated PDF has no normal appearance for field "%(name)s".',
                    name=name,
                ))
        ...
        raise ValidationError(_('The generated PDF is damaged.')) from error
